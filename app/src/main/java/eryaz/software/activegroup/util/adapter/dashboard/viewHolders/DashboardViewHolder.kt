...
class DashboardViewHolder(val binding: ItemDashboardListBinding) :
    ...
    fun bind(dto: DashboardItemDto, onItemClick: ((DashboardItemDto) -> Unit)) {

        binding.dto = dto

        dto.hasPermission.onChanged { hasPermission ->
            binding.root.visibility = if (hasPermission) View.VISIBLE else View.GONE
        }
